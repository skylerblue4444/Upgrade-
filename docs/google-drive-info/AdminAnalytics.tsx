...
import { BarChart2, TrendingUp, Users, DollarSign, Download, Calendar, Filter, Globe, Zap, Activity } from "lucide-react";
...
const KPIS = [
  { label: "Monthly Active Users", value: "284,921", change: "+12.4%", up: true },
  { label: "Monthly Revenue", value: "$2.84M", change: "+23.1%", up: true },
  { label: "Avg Session Duration", value: "24m 18s", change: "+8.7%", up: true },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black flex items-center gap-2"><BarChart2 className="h-6 w-6 text-blue-400" />Platform Analytics</h1>
          <p className="text-sm text-muted-foreground">Real-time KPIs, revenue, and growth metrics</p>
        </div>
        <div className="flex gap-2">
          {["7d", "14d", "30d", "90d"].map(r => (
            <button key={r} onClick={() => setRange(r)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${range === r ? "bg-blue-600 text-white" : "bg-muted text-muted-foreground"}`}>{r}</button>
          ))}
          <Button variant="outline" size="sm" onClick={() => toast.success("Report exported")}><Download className="h-3.5 w-3.5 mr-1.5" />Export</Button>
        </div>
      </div>
          ...
          <motion.div key={label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
            <Card className="border-border/50">
              <CardContent className="pt-4 pb-3">
                ...
                <p className={`text-xs mt-0.5 ${up ? "text-green-400" : "text-red-400"}`}>{change} vs prev period</p>
              </CardContent>
            </Card>
          </motion.div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="border-border/50">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-bold">Daily Active Users</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-bold">Daily Revenue ($)</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-bold">Revenue by Source</CardTitle></CardHeader>
          <CardContent>
            <div className="flex items-center gap-4">
              <ResponsiveContainer width={160} height={160}>
                <PieChart>
                  <Pie data={REVENUE_BY_SOURCE} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="value">
                    {REVENUE_BY_SOURCE.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                  </Pie>
                  <Tooltip formatter={(v: any) => `${v}%`} contentStyle={{ background: "#1a1a2e", border: "1px solid #333", borderRadius: 8, fontSize: 11 }} />
                </PieChart>
              </ResponsiveContainer>
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-bold">Top Countries by Users & Revenue</CardTitle></CardHeader>
          <CardContent>
              ...
              {TOP_COUNTRIES.map(({ country, users, revenue, pct }) => (
                <div key={country}>
                  <div className="flex justify-between text-xs mb-1">
                    <span>{country}</span>
                    <span className="text-muted-foreground">{users.toLocaleString()} users · ${(revenue / 1000).toFixed(0)}k rev</span>
                  </div>
                  <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8 }} className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-500" />
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* NFT + Trades Chart */}
      <Card className="border-border/50">
        <CardHeader className="pb-2"><CardTitle className="text-sm font-bold">Trading Volume & NFT Mints (Daily)</CardTitle></CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={DAILY_DATA}>
              ...
              <Tooltip contentStyle={{ background: "#1a1a2e", border: "1px solid #333", borderRadius: 8, fontSize: 11 }} />
              <Line type="monotone" dataKey="trades" stroke="#f59e0b" strokeWidth={2} dot={false} name="Trades" />
              <Line type="monotone" dataKey="nfts" stroke="#8b5cf6" strokeWidth={2} dot={false} name="NFT Mints" />
              <Legend />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
