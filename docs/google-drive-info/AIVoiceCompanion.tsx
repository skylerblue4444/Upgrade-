...
import {
  Mic, MicOff, Volume2, VolumeX, Brain, Star,
  TrendingUp, Heart, Calendar, Coins, Zap, Settings,
  MessageCircle, ChevronRight, Sparkles, Bot
} from "lucide-react";
...
const COMPANION_PERSONAS = [
  { id: "shadow", name: "Shadow", emoji: "🌑", desc: "Your default crypto-savvy AI companion", personality: "Direct, analytical, crypto-native" },
  { id: "aria", name: "Aria", emoji: "✨", desc: "Warm, empathetic life coach companion", personality: "Supportive, motivating, holistic" },
  { id: "rex", name: "Rex", emoji: "🦁", desc: "Bold trading strategist companion", personality: "Aggressive, confident, risk-tolerant" },
  { id: "luna", name: "Luna", emoji: "🌙", desc: "Calm, mindful wellness companion", personality: "Peaceful, balanced, health-focused" },
];

const QUICK_TOPICS = [
  { label: "Portfolio Update", emoji: "📊", prompt: "Give me a quick update on my portfolio" },
  { label: "Trading Advice", emoji: "📈", prompt: "What should I trade today?" },
  { label: "Dating Tips", emoji: "💘", prompt: "Help me write a great dating profile bio" },
  { label: "Daily Plan", emoji: "📅", prompt: "Help me plan my day" },
  { label: "Crypto News", emoji: "📰", prompt: "What's happening in crypto today?" },
...
const getResponse = (prompt: string): string => {
  ...
  if (p.includes("portfolio")) return RESPONSES.portfolio[Math.floor(Math.random() * 2)];
  if (p.includes("trade") || p.includes("trading")) return RESPONSES.trading[Math.floor(Math.random() * 2)];
  if (p.includes("dating") || p.includes("bio")) return RESPONSES.dating[Math.floor(Math.random() * 2)];
  if (p.includes("plan") || p.includes("day")) return RESPONSES.plan[Math.floor(Math.random() * 2)];
  if (p.includes("news")) return RESPONSES.news[Math.floor(Math.random() * 2)];
  if (p.includes("motivat")) return RESPONSES.motivation[Math.floor(Math.random() * 2)];
...
export default function AIVoiceCompanion() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "companion", text: `Hey! I'm ${COMPANION_PERSONAS[0].name}, your AI companion. I'm here 24/7 to help with trading, dating, planning, and anything else. What's on your mind?`, time: "Just now" }
  ]);
  ...
  const sendMessage = (text: string) => {
    ...
    setTimeout(() => {
      ...
      setMessages(m => [...m, { role: "companion", text: response, time: "Just now" }]);
      setTyping(false);
  ...
  const toggleListening = () => {
    if (listening) {
      setListening(false);
      const prompts = ["What's my portfolio looking like today?", "Give me trading advice for SKY4444", "Help me plan my day", "What's the latest crypto news?"];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black flex items-center gap-2"><Bot className="h-6 w-6 text-cyan-400" />AI Voice Companion</h1>
          <p className="text-sm text-muted-foreground">Your 24/7 personal AI — trading, dating, planning, and more</p>
        </div>
        <Badge className={`font-bold ${speaking ? "bg-cyan-500/10 text-cyan-400 border-cyan-500/20 animate-pulse" : "bg-muted text-muted-foreground"}`}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {COMPANION_PERSONAS.map(p => (
          <button key={p.id} onClick={() => { setPersona(p); toast.info(`Switched to ${p.name}`); }}
      ...
      <Card className="border-cyan-500/20 bg-gradient-to-br from-cyan-900/10 to-purple-900/10">
        <CardContent className="py-5 text-center">
          <motion.div
            animate={speaking ? { scale: [1, 1.1, 1], opacity: [1, 0.8, 1] } : listening ? { scale: [1, 1.05, 1] } : {}}
          ...
          <p className="text-xs text-muted-foreground mb-1">{persona.personality}</p>
          {speaking && <p className="text-xs text-cyan-400 animate-pulse">Speaking...</p>}
          {listening && <p className="text-xs text-green-400 animate-pulse">Listening...</p>}
          {typing && <p className="text-xs text-yellow-400 animate-pulse">Thinking...</p>}
        </CardContent>
      </Card>
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {messages.map((msg, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className={`max-w-[85%] px-3 py-2 rounded-2xl text-xs ${msg.role === "user" ? "bg-cyan-600 text-white" : "bg-muted/50 text-foreground"}`}>
                  {msg.text}
                </div>
              </motion.div>
            ))}
      ...
      <div className="flex gap-2">
        ...
        />
        <Button className={`h-10 w-10 p-0 ${listening ? "bg-red-600 animate-pulse" : "bg-cyan-600"} text-white border-0`} onClick={toggleListening}>
          {listening ? <MicOff className="h-4 w-4" /> : <Mic className="h-4 w-4" />}
        </Button>
        <Button variant="outline" className="h-10 w-10 p-0" onClick={() => { setMuted(!muted); toast.info(muted ? "Voice enabled" : "Voice muted"); }}>
          {muted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}
        </Button>
        <Button className="h-10 px-4 bg-cyan-600 text-white border-0 text-xs" onClick={() => sendMessage(inputText)}>Send</Button>
      </div>
    </div>
  );
}
